...
class SolarFarmSolverSim:
    ...
    def initialize_model(self):
        ...
        # Define variables
        # Define variables
        self.model.Vd = Var(self.model.indices, initialize=lambda model, i: random.uniform(41.0, 42.0))
        self.model.VBatt = Var(self.model.indices, initialize=lambda model, i: random.uniform(36.0, 38.0))
    ...
    def equation1(self, model, i):
        ...
        return (-model.I_PV_model[i] 
                + model.Is_model[i] * (exp((model.Vd[i] - V_Batt_next) / (model.alpha_model[i] * model.V_t)) - 1)
                + (model.Vd[i] - V_Batt_next) / model.R_P_model[i] 
                + model.I[current_column] == 0)
    
    def equation2(self, model, i):
        """
        Node current balance (KCL) at panel i:
        I_series + I_Block_Diode = I_column
        where
        I_series  = (VBatt[i] - Vd[i]) / R_S
        I_Block_Diode = I0 * (exp((V_next - VBatt[i])/(alpha*K*T)) - 1)
        """

        # 1) Identify column current index
        col = (i - 1) // self.total_rows + 1

        # 2) Compute V_next (grounded if last row)
        ...
        # 3) Ohmic (series resistance) current
        I_series = (model.VBatt[i] - model.Vd[i]) / model.R_S_model[i]

        # 4) Leakage (exponential) current
        #    Note: Use the stored Is_model or hard-coded I0 if needed.
        I_Block_Diode = 8.8247e-15 * (
            exp((V_next - model.VBatt[i]) /
                (model.alpha_model[i] * model.K_model[i] * model.T_model[i])) # I think you missed the multiplication by q
            - 1
        )

        # 5) KCL: sum of currents equals column current
    ...
    def equation4(self, model):
        ...
        return model.VBatt[1] == model.Z_Load_model * model.I_MPP
