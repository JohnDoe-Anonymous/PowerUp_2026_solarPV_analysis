...
class SolarFarmSolver:
    ...
    def initialize_model(self):
        ...
        I_init = 9
        I_MPP_init = 10
        self.model.Vd = Var(self.model.indices, initialize=lambda model, i: Vd_init)
        self.model.VBatt = Var(self.model.indices, initialize=lambda model, i: VBatt_init)
        self.model.I = Var(self.model.I_indices, initialize=lambda model, i: I_init)

        self.model.I_MPP = Var(initialize=I_MPP_init) 
    ...
    def equation2(self, model, i):
        ...
        I_Block_Diode = 8.8247e-15 * (
            exp((V_next - model.VBatt[i]) /
                (model.alpha_model[i] * model.K_model[i] * model.T_model[i]))
            - 1
        )
